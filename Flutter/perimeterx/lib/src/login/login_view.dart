...
class LoginView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('HUMAN Demo App'),
        actions: [
          IconButton(
  ...
  Future<void> _sendUrlRequest() async {
    try {
      final String humanHeaders = await humanPlatform.invokeMethod('humanGetHeaders');
      ...
      if (response.statusCode == 403) {
        final String result = await humanPlatform.invokeMethod('humanHandleResponse', response.body);
        print(result);
        /*
          check the result:
          'false' - not handled by the SDK
          'solved' - challenge was solved
          'cancelled' - challenge wsa cancelled
        */
        if (result == 'solved') {
          // challenge was solved. you may retry the request here
          print("challenge was solved");
        } else if (result == 'false') {
          // request was finished successfully
          print("request was finished successfully");
